...
if len(sys.argv) < 4: 
    ...
    for index, file in enumerate(entries):
        ...
        print("../tool/goeland -noeq -inner" + " ".join(sys.argv[4:]) + " " + problem_path)

        if LaunchTest("Goéland", "timeout "+timeout+" ../tool/goeland -noeq -inner" + " ".join(sys.argv[4:]) + " " + problem_path, "% RES : VALID", None, "% RES : NOT VALID"):
            cpt += 1
